...
export function ContentEditor({ initialBody = "", initialFormat = "markdown" }: { initialBody?: string; initialFormat?: Format }) {
  ...
  return <div className="space-y-3">
    <label className="text-text grid gap-1.5 text-sm font-medium">Editorformat
      <select name="format" value={format} onChange={(event) => setFormat(event.target.value as Format)} className={fieldClass}>
        <option value="rich_text">Rich Text</option><option value="markdown">Markdown</option><option value="html">HTML</option>
      </select>
    </label>
    <input type="hidden" name="body" value={body} />
    <Tabs value={activeTab} onValueChange={setActiveTab}>
      <TabsList>
        <TabsTrigger value="edit" onClick={() => setActiveTab("edit")}>Bearbeiten</TabsTrigger>
        <TabsTrigger value="preview" onClick={() => setActiveTab("preview")}>Vorschau</TabsTrigger>
      </TabsList>
      <TabsContent value="edit" className="mt-3">
        {format === "rich_text" ? <div className="border-border overflow-hidden rounded-md border">
          <div className="border-border bg-elevated flex gap-1 border-b p-2">
            <button type="button" onClick={() => command("bold")} aria-label="Fett" className="hover:bg-surface rounded p-2"><Bold className="size-4" /></button>
            <button type="button" onClick={() => command("italic")} aria-label="Kursiv" className="hover:bg-surface rounded p-2"><Italic className="size-4" /></button>
            <button type="button" onClick={() => command("insertUnorderedList")} aria-label="Liste" className="hover:bg-surface rounded p-2"><List className="size-4" /></button>
            <button type="button" onClick={() => { const url = window.prompt("URL"); if (url) command("createLink", url); }} aria-label="Link" className="hover:bg-surface rounded p-2"><LinkIcon className="size-4" /></button>
          </div>
      ...
      <TabsContent value="preview" className="mt-3">
        <div className="border-border bg-surface min-h-80 rounded-md border p-6">
          {format === "markdown" ? <div className="prose max-w-none"><ReactMarkdown remarkPlugins={[remarkGfm]}>{body}</ReactMarkdown></div> : <iframe title="Inhaltsvorschau" sandbox="" srcDoc={body} className="min-h-72 w-full border-0" />}
        </div>
      </TabsContent>
    </Tabs>
  </div>;
