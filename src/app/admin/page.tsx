...
export default async function AdminOverviewPage() {
  ...
  const stats = [
    ["Produkte", products, PackageOpen], ["Bestellungen", orders, ShoppingBag],
    ["Kunden", customers, Users], ["Rechnungen", invoices, FileText],
  ] as const;
  return <div className="space-y-8"><AdminHeader eyebrow="Betrieb" title="Übersicht" description="Aktueller Stand von Katalog, Bestellungen, Kunden und Dokumenten." />
    ...
    <Card><CardHeader><CardTitle>Letzte Bestellungen</CardTitle><CardDescription>Die fünf zuletzt angelegten Vorgänge.</CardDescription></CardHeader><CardContent>{recent.length === 0 ? <p className="text-muted py-8 text-center text-sm">Noch keine Bestellungen vorhanden.</p> : <ul className="divide-border divide-y">{recent.map((order) => <li key={order.id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><strong>{order.order_number}</strong><span className="text-muted ml-2 text-sm">{order.customer_name}</span></div><div className="flex items-center gap-3"><Badge>{order.status}</Badge><span className="font-mono text-sm">{formatPrice(order.total_cents)}</span></div></li>)}</ul>}</CardContent></Card>
  </div>;
}
