...
describe("invoice PDF", () => {
  ...
  it("lays out a deposit (Anzahlung) split on a standalone invoice", async () => {
    const { generateInvoicePdf } = await import("./pdf");
    const bytes = await generateInvoicePdf({ invoiceNumber: "RE-2026-000004", issuedAt: "2026-08-10T12:00:00.000Z", issueDate: "10.08.2026", dueDate: "24.08.2026", orderNumber: null, taxRate: 19, company: { name: "holz direkt GmbH - Holzimport", legalForm: "GmbH", street: "Bergweg 24", postalCode: "48485", city: "Neuenkirchen", countryCode: "DE", vatId: "DE813362690" }, customer: { name: "Acompte Kunde", street: null, houseNumber: null, postcode: null, city: null }, items: [{ name: "Pelletofen ivo.tec water+", quantity: 1, unitPriceCents: 200000, lineTotalCents: 200000 }], amounts: { subtotalCents: 200000, discountCents: 0, shippingCents: 0, taxCents: 31933, totalCents: 200000, depositCents: 60000, depositPercent: 30 } });
    expect(bytes.byteLength).toBeGreaterThan(1000); await writeFile("tmp/invoice-deposit.pdf", bytes);
  }, 30_000);
