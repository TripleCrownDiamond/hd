#!/usr/bin/env node
/**
 * Scrape brennio.de (Vergleichsportal für Brennstoffe) wood offers.
 *
 * - React SPA: listing pages ship an ItemList JSON-LD with offer URLs
 *   like /guenstig-kaufen/<slug>/<filter>/ — those are server-rendered pages
 * - Discover: crawl /shop/brennholz/ + essence subpages (_buche, _birke, …),
 *   parse ItemList JSON-LD, collect /guenstig-kaufen/ offer URLs, dedupe
 * - Offer pages: h1 = full model (essence - length - drying - pack), price,
 *   unit, cm — extracted from title/h1 + page text
 *
 * Output: data/scraped/brennio/{yyyy-mm-dd}.jsonl
...
const SOURCE = "brennio";
const BASE = "https://www.brennio.de";
...
function offerPrice(html) {
  // price is announced in the <title> and possibly meta: "ab 12,50 €"/"ab 12,50&nbsp;€"
...
async function scrapeOffer(url) {
  ...
  const { cents, raw } = offerPrice(body, h1);
